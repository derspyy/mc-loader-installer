...
pub fn install(minecraft_version: String, loader_version_opt: Option<String>, mut minecraft_directory: PathBuf) -> Result<(), Box<dyn error::Error>> {
    ...
    let json_data = agent.get(format!("{QUILT_META}/loader/{minecraft_version}/{loader_version}").as_str())
        .call()?;
    minecraft_directory.push("versions");
    minecraft_directory.push(format!("quilt-loader-{loader_version}-{minecraft_version}").as_str());
    ...
    minecraft_directory.push(format!("quilt-loader-{loader_version}-{minecraft_version}.json").as_str());
...
fn get_latest(agent: &Agent) -> Result<String, Box<dyn error::Error>>{
    let request = agent.get(format!("{QUILT_META}/loader").as_str())
        .call()?
        .into_string()?;
    ...
    version.ok_or(Box::new(Error::NoVersion))
}